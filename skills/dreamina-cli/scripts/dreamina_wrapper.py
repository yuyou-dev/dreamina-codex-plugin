...
def validate_seedance_video_model(namespace: argparse.Namespace, command_name: str) -> None:
    model_version = namespace.model_version or "seedance2.0fast"
    if model_version not in SEEDANCE2_MODELS:
...
COMMAND_SPECS: dict[str, CommandSpec] = {
    ...
    "text2video": CommandSpec(
        ...
        examples=(
            'python3 scripts/text2video.py --prompt "camera slowly pushes toward a diamond necklace" --duration 5',
        ),
    ),
    ...
    "multiframe2video": CommandSpec(
        ...
        parameters=(
            ...
            ParameterSpec("duration", "duration", "Shorthand duration for exactly two images.", value_type="float", min_value=0.5, max_value=8),
            ParameterSpec("transition_prompt", "transition-prompt", "Repeat once per transition segment.", multiple=True),
            ParameterSpec("transition_duration", "transition-duration", "Repeat once per transition segment.", value_type="float", multiple=True, min_value=0.5, max_value=8),
            ParameterSpec("session", "session", "Dreamina session ID.", value_type="int", min_value=0),
            ParameterSpec("poll", "poll", "Optional polling window in seconds.", value_type="int", min_value=0),
        ),
        examples=(
            'python3 scripts/multiframe2video.py --images ./a.png,./b.png --prompt "the character turns to face camera"',
            'python3 scripts/multiframe2video.py --images ./a.png --images ./b.png --images ./c.png --transition-prompt "A turns into B" --transition-prompt "B turns into C"',
        ),
    ),
    "multimodal2video": CommandSpec(
        ...
        examples=(
            'python3 scripts/multimodal2video.py --image ./scene.png --audio ./music.mp3 --model-version seedance2.0fast --duration 5',
        ),
    ),
    ...
    "checklogin": CommandSpec(
        name="checklogin",
        description="Check a prior headless OAuth Device Flow login by device_code.",
        output_mode="text",
        cli_command=("login", "checklogin"),
        parameters=(
            ParameterSpec("device_code", "device_code", "Device code printed by login.py --headless or relogin.py --headless.", required=True),
            ParameterSpec("poll", "poll", "Wait for up to N seconds; 0 checks once.", value_type="int", min_value=0),
        ),
        examples=(
...
def build_cli_args(spec: CommandSpec, namespace: argparse.Namespace) -> list[str]:
    args = list(spec.cli_command or (spec.name,))
    for parameter in spec.parameters:
        value = getattr(namespace, parameter.key)
